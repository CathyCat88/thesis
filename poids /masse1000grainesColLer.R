...
contLer <- data.frame(read.csv("Ler1000.csv", sep = ',', dec = '.', header = TRUE))
contCol <- data.frame(read.csv("Col1000.csv", sep = ',', dec = '.', header = TRUE))
...
air12Ler <- data.frame(contLer[which(contLer$genotype == "air 12"), ])
...
t.test(WTLer$masse, nqrLer$masse, var.equal = TRUE) # égalité des moyennes
t.test(WTLer$masse, air12Ler$masse, var.equal = TRUE) # égalité des moyennes
t.test(WTLer$masse, nqrair12Ler$masse, var.equal = TRUE) # égalité des moyennes
t.test(WTCol$masse, nqrCol$masse, var.equal = TRUE) # non égalité des moyennes
...
g
save_plot('masse1000graonesLerCol.png', g, base_aspect_ratio = 1.3)
